...
export default function PracticePage() {
  ...
  const fetchHistory = async (sentenceId: number, force?: boolean) => {
    if ((practiceHistory[sentenceId] !== undefined || historyLoading[sentenceId]) && !force) return;
  ...
  const handleAnswerChange = (val: string) => {
    ...
    try {
      ...
      practiceDataRef.current = updated;
      setPracticeData(updated);
      const prev2 = { ...practiceHistory }; delete prev2[current!.id]; setPracticeHistory(prev2);
      setTimeout(() => fetchHistory(current.id), 100);
                                ...
                                {practiceHistory[current.id].map((h, hidx) => {
                                  ...
                                  return (
                                    <div key={h.id} className="bg-[#FAFAF7] rounded-2xl p-3 border border-[#F0F0EC]">
                                      ...
                                      <div className="text-xs text-[#2D3436] font-medium mb-1.5 leading-relaxed">
                                        {f && f.errors && f.errors.length > 0
                                          ? (() => {
                                              const parts = highlightErrors(h.answer, f.errors);
                                              return parts.map((p, pi) => p.hasError
                                                ? <span key={pi} className="bg-[#FFE0E0] text-[#FF6B6B] px-0.5 rounded border border-[#FF8A8A]/30">{p.text}</span>
                                        ...
                                        <div className="flex flex-wrap gap-1 mb-1">
                                          {f.errors.map((err, ei) => (
                                            <span key={ei} className="text-[10px] px-1.5 py-0.5 bg-[#FFF5F5] text-[#FF8A8A] rounded-lg">
                                              ...
                                              <span className="line-through">{err.wrong}</span>
                                              <span>→</span>
                                              <span>{err.correct}</span>
                                            </span>
                                          ))}
                                        </div>
                                      )}
