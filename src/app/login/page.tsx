    ...
    <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-[20px] p-8 border border-[#F0F0EC]" style={{ boxShadow: "0 8px 30px rgba(0,0,0,0.04)" }}>
          <h2 className="font-semibold text-[#2D3436] mb-6">欢迎回来</h2>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full px-4 py-3 bg-[#FAFAF7] border border-[#F0F0EC] rounded-2xl outline-none focus:ring-2 focus:ring-[#8FD8B5]/30 focus:border-[#8FD8B5] transition-all text-sm" placeholder="请输入密码" required />
            </div>
            <button type="submit" disabled={loading} className="w-full bg-[#8FD8B5] text-white py-3 rounded-[16px] font-semibold hover:bg-[#6BBF99] transition-all disabled:opacity-50">{loading ? "登录中..." : "登 录"}</button>
          </form>
          <p className="text-center text-sm text-[#B2BEC3] mt-6">还没有账号？<Link href="/register" className="text-[#8FD8B5] hover:text-[#6BBF99] font-medium">注册</Link></p>
        </div>
      </div>
    </div>
  );
